...
function drop(ev) {
  ...
  var fieldRect = fieldImage.getBoundingClientRect(); // Get the position of the field image
  var offsetX = ev.clientX - fieldRect.left; // Calculate the offsetX relative to the field image
  var offsetY = ev.clientY - fieldRect.top; // Calculate the offsetY relative to the field image
  nodeCopy.style.position = "absolute";
  nodeCopy.style.left = offsetX + "px"; // Set the left position relative to the field image
  nodeCopy.style.top = offsetY + "px"; // Set the top position relative to the field image
  fieldImage.appendChild(nodeCopy);
}
</script>
"""
